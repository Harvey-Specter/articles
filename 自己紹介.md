...
御社の主要な業務はクラウドセキュリティサービスであり、私が以前に取り組んでいたブロックチェーンプラットフォームの主要(しゅよう)な機能もデータとプログラムのセキュリティを保護(ほご)することでした。
...
この製品は AWS に加Kuwaえて、他のクラウド プラットフォームもサポートしていますか?
...
弊社は日本に支社を持っていますが、日本支社のポジションは主にマーケティングやセールス関連が多いようです。私は専門(せんもん)的に技術に携(たずさ)わりたいと考えており、そのための専門の部署(ぶしょ)が存在(そんざい)しないようです。私にとって適切な職種(しょくしゅ)が見当(みあ)たらないため、技術関連の仕事に従事(じゅうじ)したいと思っています。

御社はベンチャー企業であり、大きな成長余地(せいちょうよち)とポテンシャルがあります。私は教育業界のアプリケーションにも興味があります。なぜなら、私の娘が小学校に通っているからです

## プロローグ

私は旅行(りょこう)が好きな人ではありません。もし私がある場所が好きなら、その場所で生活したいと思います。そうしないと、その地域(ちいき)の魅力を存分(ぞんぶん)に感じることはできないと考えています。

申し訳ございませんが、私の都合により、時間を変更させていただきました。
お待たせして申し訳ございません。
...
今回、私は GreenJapan 上(じょう)で御社のポジションに応募(おうぼ)しました。私は中国出身のソフトウェアエンジニアです。過去(かこ)にはさまざまなソフトウェア開発プロジェクトに参加し、異なるプログラミング言語や開発ツールを使った経験を積(つ)んできました。御社を選(えら)んだ理由は、技術的な評判(ひょうばん)や将来性(しょうらいせい)に魅力を感じたためです。御社は革新(かくしん)的な取り組みやチームワーク、成長の機会(きかい)を提供しているということを知りました。これが私が希望する環境です。
...
私は英語のレベルが、日本語よりも少し高いかもしれないと思っています。学生時代からずっと英語を学んでおり、仕事でも英語を使っていますので、英単語(えいたんご)の知識(ちしき)も多いはずです。ただし、ここ数(すう)ヶ月は日本語の会話力を鍛(きた)えるために努力しており、現在は日本語の会話の方(ほう)が英語よりも得意だと思います。

私は友達と山登(さんとう)りやサッカーをすること、そしてカラオケ(卡拉OK)に行くことが好きですが、最近は仕事の合間(あいま)に日本語を勉強し、仕事探し(さがし)もしているので、遊(あそ)に行(い)くことが久(ひさ)しぶりです
